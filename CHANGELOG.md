...
- **Bundle id renamed** from `com.bistin.dynamic-island` to
  `com.cc-island.dynamic-island` so forks of the repo aren't
  pinned to the original maintainer's namespace. **Breaking for
  existing users:** `~/Library/Preferences/com.bistin.dynamic-island.plist`
  does not migrate to the new domain — on first launch after
